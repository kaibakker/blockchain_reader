...
class Block
  attr_accessor :magic_number, :blocksize, :version, :previous_hash, :merkle_hash, :time, :bits, :nonce, :transactions_count, :transactions
  ...
  def initialize(blockchain)
    ...
    @transactions_count.times do
      @transactions << Transaction.new(blockchain)
    end
  end
...
class Transaction
  attr_accessor :version, :inputs_count, :inputs, :outputs_count, :outputs, :lock_time
  def initialize(blockchain)
    @version = uint4(blockchain)
    ...
    @inputs_count.times do
      @inputs << TransactionInput.new(blockchain)
    end

    @outputs_count = varint(blockchain)
    @outputs = []
    @outputs_count.times do
      @outputs << TransactionOutput.new(blockchain)
    end
...
class TransactionInput
  attr_accessor :previous_hash, :transaction_output_id, :script_length, :script_sign, :sequal_number
  ...
  def to_s
    ...
    string += "Script Length:\t#{@script_length}\n"
